...
import {type ClockStateDateTime, getClockState} from './clock-state.ts';
...
export const clock: FastifyPluginAsyncZod = fastifyPlugin(
  ...
  async (fastify, options) => {
    ...
    fastify.decorate('clockState', getClockState(fastify.settings));
    ...
    function updateClockState() {
      fastify.clockState = getClockState(fastify.settings);
